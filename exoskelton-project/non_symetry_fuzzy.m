clear; clc;
load('robot_setup.mat');                                  % Robot definition
load('trajectories.mat', 'desiredTrajectories', 'time', 'timeStep');  % Left Leg
load('nonidealSymmetricHip.mat', 'R_hip_nonideal');       % Corrected filename
load('nonidealSymmetricKnee.mat', 'R_knee_nonideal');     % Corrected filename
fuzzyController = readfis('fuzzy.fis');                   % Load FIS

%% === Trim all to common length ===
minLength = min([length(time), length(R_hip_nonideal), length(R_knee_nonideal), size(desiredTrajectories,1)]);
time = time(1:minLength);
desiredTrajectories = desiredTrajectories(1:minLength, :);
R_hip_nonideal = R_hip_nonideal(1:minLength);
R_knee_nonideal = R_knee_nonideal(1:minLength);

% Combine all 4 joints: [L_Hip, L_Knee, R_Hip, R_Knee]
desiredTrajectories = [desiredTrajectories(:, 1:2), R_hip_nonideal, R_knee_nonideal];
...
%% === SAVE OUTPUT ===
save('fuzzy_pid_output_full_leg_nonideal.mat', 'actualTrajectories', 'time', 'timeStep');
disp('✅ Fuzzy PID simulation completed and saved for 4 joints (non-ideal symmetry).');

%% === PLOT RESULTS ===
jointLabels = {'Left Hip', 'Left Knee', 'Right Hip (Non-Ideal)', 'Right Knee (Non-Ideal)'};
...
print(gcf, 'fig_fuzzy_pid_full_leg_nonideal.pdf', '-dpdf', '-bestfit');
disp('✅ Full-leg fuzzy PID plot (non-ideal symmetry) saved to PDF.');
